...
load("C:/Users/Russell Boag/Documents/GitHub/DMCATC/data/samples/A1.block.B.V_cond.B.V.PMV.samples.RData")  # Samples object
...
CDT <- data.A1[!(data.A1$S=="pc" | data.A1$S=="pn"),]
CDT$S <- factor(as.character(CDT$S)); CDT$R <- factor(as.character(CDT$R))
str(CDT)
head(CDT)

# PM Task trials only
PMT <- data.A1[(data.A1$S=="pc" | data.A1$S=="pn"),]
PMT$S <- factor(as.character(PMT$S)); PMT$block <- factor(as.character(PMT$block))
str(PMT)
head(PMT)
#

CDT.Acc.glmer.A1 <- glmer(C ~ S*block*cond+(1|s), data=CDT, family=binomial(link="probit"))
save(CDT.Acc.glmer.A1, file="CDT.Acc.glmer.A1.RData")
load("CDT.Acc.glmer.A1.RData")
CDT.Acc.glm.A1 <- Anova(CDT.Acc.glmer.A1,type="II")
CDT.Acc.glm.A1
...
PMT.Acc.glmer.A1 <- glmer(C ~ S*cond+(1|s), data=PMT, family=binomial(link="probit"))
save(PMT.Acc.glmer.A1, file="PMT.Acc.glmer.A1.RData")
load("PMT.Acc.glmer.A1.RData")
PMT.Acc.glm.A1 <- Anova(PMT.Acc.glmer.A1,type="II")
PMT.Acc.glm.A1
...
#
PMC.RT.lmer.A1 <- lmer(y ~ S*cond+(1|s), data=dPMT.RT)
...
CDT.RT.corr.BLOCK <- ddply(data.A1[ (data.A1$S=="cc" & data.A1$C=="1") |
                                        (data.A1$S=="nn" & data.A1$C=="1"), ],
                           ~S*block, summarise, M=mean(RT, na.rm = TRUE), SD=sd(RT, na.rm = TRUE))

# PM Task Correct RTs by Block
PMT.RT.corr.BLOCK <- ddply(data.A1[ (data.A1$S=="pc" & data.A1$C==1) |
                                        (data.A1$S=="pn" & data.A1$C==1), ],
                           ~block, summarise, M=mean(RT, na.rm = TRUE), SD=sd(RT, na.rm = TRUE))

# Ongoing Task Correct RTs by Condition
CDT.RT.corr.COND <- ddply(data.A1[ (data.A1$S=="cc" & data.A1$C==1) | (data.A1$S=="nn" & data.A1$C==1), ], ~S*cond, summarise,
      M=mean(RT, na.rm = TRUE), SD=sd(RT, na.rm = TRUE))

# PM Task Correct RTs by Condition
PMT.RT.corr.COND <- ddply(data.A1[ (data.A1$S=="pc" & data.A1$C==1) | (data.A1$S=="pn" & data.A1$C==1), ], ~cond, summarise,
                     M=mean(RT, na.rm = TRUE), SD=sd(RT, na.rm = TRUE))
