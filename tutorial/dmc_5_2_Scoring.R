...
#
# You could also do it programatically using the elements of the "map" names 
...
# "assign.map" has a "funs" arguement specifying a list of lists that give
...
#
# For "dm" both equalites must hold. "assign.map" allows two or more equality
...
# This is the default behavior of "assign.map" so is accomplished by
...
#
# Suppose that R had levels with full names. Here an inline function can be
# used to pick out the first letter for testing
map <- empty.map(list(RS=c("R","G","B"),IS=c("r","g","b"),
         R=c("red","green","blue")),levels=c("dm","rm","im","nm"))
funs=list(list("tolower",function(x){substr(x,1,1)}),
          list("",       function(x){substr(x,1,1)}))
map <- assign.map(map,value="rm",eq.list=list(c(1,3)),funs=funs[-2])
map <- assign.map(map,value="im",eq.list=list(c(2,3)),funs=funs[-1])
map <- assign.map(map,value="dm",eq.list=list(c(1,3),c(2,3)),funs=funs)
map <- assign.map(map,value="nm")
#
# R.r.red   G.r.red   B.r.red   R.g.red   G.g.red   
#    "dm"      "im"      "im"      "rm"      "nm"
# ...
...
model <- model.dmc(
  factors=list(RS=c("R","G","B"),IS=c("r","g","b")),
  responses=c("red","green","blue"),
  p.map=list(A="1",B="1",t0="1",mean_v="RI",sd_v="1",st0="1"),
  match.map=list(M=list(R="red",G="green",B="blue"),RI=map),
  constants=c(sd_v=1,st0=0) )
