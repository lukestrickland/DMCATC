...
# estimates from models estiamted without fitting the covariates, see 
...
# was X and it had slope coefficeint s then the data froam would contain the
# the product sX. 
...
# Traditional correlation tests between the covariate and  the posteior mean. 
# The parameter estiamtes are not independent, violating test assumptions and
...
# uncertianty from subjects being samples from a population (subjects as 
# "random effects") and so is not suitbale for population inference. 
# The following demonstrates how to obtain population plausibel vlaeus based on
...
# default set to 1 implyig a uniform prior.
kappa <- 1 # Uniform prior

# To Cacluates density values from -1 to 1 at regular intervals given
...
# These graphs show the effect of taking population uncertianty into account
par(mfrow=c(1,2))
...
#    peaked the distribution, with the requested values being estiamted via 
#    linear inerpolation. Here we check that by using a higher resolution. 
