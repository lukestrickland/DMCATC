...
# Here we compare two LNR models for a design with a stimulus manupulation (S)
...
# Goodness of fit can be assessed by plotting the cdf for the emprical data
# and posterior predictives averaged over subjects. 

# NB: There is something wrong with this averagign procedure as evident in the 
#     misfit of plot.pp.dmc(pp11) which is not present at the indivdiual subject
#     level. New plotting is being developed to address this problem.
...
# doesnt save much time, but in slower cases it may help, although be careful 
...
# log-likelihods matrix (i.e., points are subjects). Remember to only use
...
# The usual waic function can then be used to calcualte the group waic
M0D0_G <- waic.dmc(group_trial_0.0,save=T)
# Again the usual waic function can then be used to calcualte the group waic
...
# of about 6. This demonstrates the stability we loose when using subjects in 
...
# unless fast=FALSE, in which case the posterior likelihoods are re-calcualted.

# By default BPIC is calcualted:
...
# The true model is chosen by a wide margin 164057.7-163895.5 = 162.2
