...
# NB: You can seperately tune gamma for hierarchical (h.gamma.mult) and subject 
...
# samples (recall gelman.diag can be high simply becasue the series is short).
...
# The credible intervals on the hyper estiamtes are given by
...
# You can plot only the location parameters using
pairs.dmc(hsamples1,hyper=TRUE,hyper2=FALSE,thin=5)

# or only the scale paraemters
pairs.dmc(hsamples1,hyper=TRUE,hyper1=FALSE,thin=5)
...
# Note that you can even remove non-contiugous sections, so, for example, you 
...
# parameter recovery is only slighty worse. Running this is left as an exercise. 
...
# the intermediate versions ofthe returned object (here hsamples1.100). This can 
# be useful because the runs can be long so you do not loose to much work in 
# case of a crash.
