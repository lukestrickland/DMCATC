...
# Here we see that the nominal number of samples, 500*15 = 7500 is effectivley
...
# specifying the max.lag argument. Plotting can be turned off and the calcualted 
...
# autocorrelation approches zero then the samples become close to independent.
...
# Thinning can be applied direclty during sampling. Here we thin at k=10, which
...
# The reduced autocorrelation is evident in a comparision of the posteior
# log likelihood plot for the last 200 samples wihtout thining
...
# also includes an indication of uncertianty in the model, grey lines 
...
# order as reponses are listed in the legend. 
...
# odd ones. When there are lots of responses the legend can also become unweildy.
...
# x.min.max, a 2-vector of minimum and maximum values (if the calcualed xlim
...
#      RT quantile, means and SDs as well as accuracy for followup checks. This
...
# in estatimes (Median estimated minus true). Note estimation is said have
...
# has been well estimatimated. In this case it coverage was achieved in all
# cases (note that we had to set digits=3 as at the defualt 2 everything is
# 0.2 for t0)
...
# This function can also be used to produce a nicely formated summary in 
# fits to real data by simply ommiting the true paramter vector.
...
# Clearly the updating is very strong as the prior (red lines) is completley 
# dominated by the posterior. show.obs=FALSE jsut turns of CODAs default "rug"
# plot which


# You can also view the degree of correlation among parameters using a pairs
# plot, for the LNR this corelation is fairly mild.
pairs.dmc(samples4)
