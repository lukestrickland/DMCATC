...
par(mfcol=c(2,4)) 
for (i in 1:2) { # Upper=biased to resposne, Lower = biased away. First column = greater rate, second lesser
...
# the number of iteraitons for that subject is printd; it will take a while!

# We will do some timing to be used later on when considering heirarchical fitting.
system.time({samples  <- h.run.unstuck.dmc(samples, p.migrate = .05, cores = 27)})
#    user  system elapsed 
#  65.575  14.186 157.742 

# Now run without migartion, and turning up thinning to 10
...
# Paramter chains look generally well converged and mixed, but with the
...
# as actual n divided by either mimimum or mean effective n. 
get.thin(samples1)
...
# but here we mainly use as a preamble to hierarchcial fitting so will stop here.
...
# Check parameter recovery with the following conveneince function which takes
...
# Now lets try using start values from the fixed effects run.
...
# ranodm effect fits (one core per chain, where each chain is quick so there
...
# for what might be used in hierarhcial sampling, but it is probaly best to
...
# Thses values are uniformly closer to true values than fixed effect estimates.
...
# funciton is assigned the printed matrix is saved.

# This funtion can also be used to look at the average of indivdiual parameter 
# recovery, which is mostly similar to the hyper.
