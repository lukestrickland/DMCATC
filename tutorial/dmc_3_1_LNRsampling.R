...
# In a Bayesian context we fit a model and obtain estiamtes of its parameters
...
# with farjump=10 (i.e., every 10th interation).
...
# Before we finish note that samples.dmc allows you selectivley remove samples
# using the "remove" argument, which specifies a range of samples that will be
# omitted. 

# To preseve the remaining samples and not include room for any new samples 
# you use add=TRUE with nmc=0 (of cousre if you want to add room for new samples
...
# On occasion you may have knowledge of the likley range of values of a parameter
