...
# in LBA. This allows the flexibility to have seperate factor specifications
# for match and mismatch (i.e., mean_v.true and mean_v.false) rates. To do so
# we create a new external parameter mean_v_false (any name can be used as long
# as it is appropriately dealt with by transform.dmc).

load_model ("LBA","lba_B2v.R")
model <- model.dmc(
  p.map=list(A="1",B="1",mean_v=c("S","M"),mean_v_false="1",sd_v=c("M"),t0="1",st0="1"),
  constants=c(st0=0,sd_v.true=1,
    mean_v.s1.false=Inf),
...
# Note that you could use a similar approach to have seperate parameters for 
