...
# here and the hierarhical estimation described in the next lesson, with some 
...
# You can convenienlty look at many subjects at once to check for stuck chains
...
# Look at how much the effectiveSize differs from the actual nuber of samples 
...
# Suppose we wanted a mean over parametrs of at least 700 effective samples. 
...
# Note, this can be run in a parallel mode, with subjects run on seperate cores, 
...
# is supplied as an argument. It is calcualted by averaging data from all
...
# There are two ways to do this. One concatanates trial from each subject to 
# create internations by trials pointwise log-likelihood matrix (i.e., points
# are trials). 

# First extract and concatanate. This can create a very big matrix so some  
...
# numbers of interations for some participants. The same number are used for 
