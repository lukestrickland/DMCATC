...
# INDEPENDENT paramterization (i.e., seperate stop accumulator parameters).
# 2 accumualtor race suitable for high accuracy data like BEESTS. 
...
plot_SS_if.dmc(data = data)  #P(Respond) increases as a function of SSD, as it should
Ns
...
# Plot median signal-respond RT per SSD:
plot_SS_srrt.dmc(data = data) #Median SRRT increases as a function of SSD, as it should
...
# Go parameters are typically better contrained than stop parameters (more data = less uncertainty)
...
plot_SS_if.dmc(data=samples2$data,fit=TRUE,sim=pp)
# SSD  n     p
# 0.10  1    NA
# 0.15  6 0.560
# 0.20 18 0.215
...
plot_SS_srrt.dmc(data=samples2$data,fit=TRUE,sim=pp)
# SSD  n nrt         p n.sim
...
par(mfcol=c(1,2))
plot_SS_if.dmc(data=samples4$data,fit=TRUE,sim=ppMis)
plot_SS_if.dmc(data=samples2$data,fit=TRUE,sim=pp)

par(mfcol=c(1,2))
plot_SS_srrt.dmc(data=samples4$data,fit=TRUE,sim=ppMis,ylim=c(.3,.8))
plot_SS_srrt.dmc(data=samples2$data,fit=TRUE,sim=pp,ylim=c(.3,.8))
