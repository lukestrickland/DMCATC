...
# The second element contains RT quantiles, for example the median, which is the 
...
# probs.gglist 
#The third and fourth element contain posterior predictives for mean RT,
# and SD of RTs, respectively.
 
# The example uses a simple LNR example, from lesson 4.4,
...
# h.post.predict.dmc saves a gglist by default, which is an attribute of the 
...
# The ggplot2 package provides researchers with a flexible, intuitive graphics
...
# Note that a theme is set when dmc is sourced, by funning the function 
# theme_simple.
...
correctonly.df <- lnr.group.gglist[[2]]
...
df.newlabels <- lnr.group.gglist[[2]]
...
df.neworder <- lnr.group.gglist[[2]]
...
ggplot.RT.dmc(lnr.group.gglist[[3]], do.quantiles=FALSE) +ylab("Mean RT")
ggplot.RT.dmc(lnr.group.gglist[[4]], do.quantiles=FALSE) +ylab("SD RT")
