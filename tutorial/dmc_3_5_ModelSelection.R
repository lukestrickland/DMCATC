...
# to perform model seleciton, favoring the model that provides a good fit while
...
exp(-d/2)/sum(exp(-d/2))
# So a difference of 6 means you are >95% in favor of a model.
...
# prohibitve, so loo provides two approximations. The following paper describes
# and tests them relative to full Bayesian leave-out-one cross-validaiton as the
# gold standard.
...
# this occurs. The "looic.dmc" function can be used to calcualte looic and 
...
#   129 (0%) Pareto k estimates estimates between 0.5 and 1
