...
### Lesson 4.6_group: Hierarchical LBA Model with a simple design and smaller
### sample size where full hierarhcial fails (for the same design
### with twice the sample size hierarchcial works). Convergence problems
...
### casue more problems. Chain update mixing in DMC (defualt random.phi=TRUE and 
### random.theta=TRUE) sovles the convergence problem, but parameter recovery 
### still difficult.
...
### fixesconvergence problems identified in the next section, see also 4_7group.
...
# Need a new model in which sets A to a constant in model (value doesnt matter)
...
# Note that there is plot of scale for A as not estimated. A parmater looks less variable.
...
# However some A and B problems, and mean_v.false.h2 displays zero varinace trap
...
# Relativley short series due to high autocorrelation
...
# DMC can do several group parameters at once. Here we explroe whether making
# mean_v.false a group parameter to see if that fixes estimation problems, 
# turns out to make things worse. Group parameters may only be suitble for
# trial to trial variability parameters that have weak influnece on data. 
...
# Strong decrese in likleihoods then chains not mixing well, 
