...
# made to determine the probability of obtaining a differnce of the observed
# magnitude (i.e., a posterior p value). This requires calcualting the differnce
# for each sampled posterior parameter and calculating the probability that the
# distirbution of differences is greatr than zero.
# 
# The procedure can be applied to individual subjects fits, fixed effects fits
...
# not presently built in to DMC's hierarhcical fitting (it is planned for the
# future). So, it is recommended that tests of within-subject factors are 
# done on averages of indivdiual subject parameters, as they do capture the 
...
# as an example, with samples (from a hierarchcial fit) contained in an 
# object called "hsamples".
load_data("dmc_5_6.RData")

# The experiment had 20 subjects who had to indiciate which of stwo stimului 
# occured (factor S=stimulus: low or high). They could do this either while 
...
# subjects effect, averaging over particiapnt differences for two parameters
...
# off with show.table=FALSE). Defult line0=TRUE shows a vertical line at zero.
# The legend can be turned off (show.legend=FALSE) and its position contolled
...
# the gf estiamtes to the probability scale (they are sampled on the probit)
# scale) before taking the difference. Note the funciton must return a 3-vector
# containing each element of the differnce and the differnce itself. The 
...
# testing location), showign greater indivdiual differenes in go failures under
# load.
...
# Returning to test of averages over indivdiual parameters, here we use a 
