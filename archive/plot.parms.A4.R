...
samples <- A4.block.B.V_cond.B.V.PMV.samples
...
# gelman.diag.A4 <- gelman.diag.dmc(samples)
# save(gelman.diag.A4, file="gelman.diag.A4.RData")
load("gelman.diag.A4.RData")
...
A4PP <- h.post.predict.dmc(samples, save.simulation=
                             TRUE)
save(A4PP, file="data/after_sampling/A4PP.RData")
load("data/after_sampling/A4PP.RData")

sim <- do.call(rbind, A4PP)
# Do the same for the data
data <- lapply(A4PP, function(x) attr(x, "data"))
...
# ggsave("A4.Fits.Acc.Ongoing.png", plot = last_plot())
...
# ggsave("A4.Fits.Acc.PM.png", plot = last_plot())

# The grid arrange will depend on your design
A4.acc.plots <- grid.arrange(ongoing.acc.plots, PM.acc.plots,layout_matrix = cbind(
  c(1,1,2), c(1,1,2)))

ggsave("A4.Fits.Acc.png", plot = A4.acc.plots, width = 9, height = 12)
...
# ggsave("A4.Fits.RT.Correct.png", plot = last_plot())
...
# ggsave("A4.Fits.RT.Error.png", plot = last_plot())
...
# ggsave("A4.Fits.RT.PM.png", plot = last_plot())
...
A4.RT.plots <- grid.arrange(corr.RTgraph, err.RTgraph, PM.RTgraph, layout_matrix = cbind(
  c(1,1,2,2,3), c(1,1,2,2,3)))

ggsave("A4.Fits.RT.png", plot = A4.RT.plots, width = 9, height = 12)
#


# Do out of sample predictions of non-responses to see whether they are
# consistent with the model.
load("data/exp_data/okdats.A4.NR.RData")
...
save(h.ordermatched.sims,
     file="data/after_sampling/A4NRPP.RData")
load("data/after_sampling/A4NRPP.RData")
...
ggsave(NR.plot, file="A4.NR.Prob.png", width = 9, height = 6)
...
# av.thetas.A4 <- fixedeffects.meanthetas(samples)[[1]]
# save(av.thetas.A4, file="av.thetas.A4.RData")
load("av.thetas.A4.RData")

msds <- cbind(apply(av.thetas.A4, 2, mean), apply(av.thetas.A4, 2, sd))
...
setwd("C:/Users/Russell Boag/Documents/GitHub/DMCATC")
ggsave("A4.Thresholds.png", plot = last_plot(), width = 9, height = 6)
...
# ggsave("A4.Rates.Correct.png", plot = last_plot())
...
#
# ggsave("A4.Rates.Ongoing.Correct.png", plot = last_plot())
...
# ggsave("A4.Rates.Ongoing.FA.png", plot = last_plot())
...
# ggsave("A4.Rates.Reactive.Inhibition.png", plot = last_plot())
...
ggsave("A4.Rates.png", plot = V.plots, width = 9, height = 12)
