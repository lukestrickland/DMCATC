...
# # # Load samples object # # #
#
load("C:/Users/Russell Boag/Documents/GitHub/DMCATC/samples/A2.block.B.V_cond.B.V.PMV.samples.RData")  # Samples object
...
Reactive.Table <- data.frame(rbind(
    A.NonPM.minus.PM=c(zandp(samples.A2, Reactive.ccAC),
        zandp(samples.A2, Reactive.nnAN),
        zandp(samples.A2, Reactive.nnAC),
        zandp(samples.A2, Reactive.ccAN)
        ),

    B.NonPM.minus.PM=c(zandp(samples.A2, Reactive.ccBC),
        zandp(samples.A2, Reactive.nnBN),
        zandp(samples.A2, Reactive.nnBC),
        zandp(samples.A2, Reactive.ccBN)
        ),

    C.NonPM.minus.PM=c(zandp(samples.A2, Reactive.ccCC),
        zandp(samples.A2, Reactive.nnCN),
        zandp(samples.A2, Reactive.nnCC),
        zandp(samples.A2, Reactive.ccCN)
        ),

    D.NonPM.minus.PM=c(zandp(samples.A2, Reactive.ccDC),
        zandp(samples.A2, Reactive.nnDN),
        zandp(samples.A2, Reactive.nnDC),
        zandp(samples.A2, Reactive.ccDN)
        )
))
...
Proactive.Table <- data.frame(rbind(
    A.PM.minus.Control=c(zandp(samples.A2, Proactive.AC),
        zandp(samples.A2, Proactive.AN)
    ),

    B.PM.minus.Control=c(zandp(samples.A2, Proactive.BC),
        zandp(samples.A2, Proactive.BN)
    ),

    C.PM.minus.Control=c(zandp(samples.A2, Proactive.CC),
        zandp(samples.A2, Proactive.CN)
    ),

    D.PM.minus.Control=c(zandp(samples.A2, Proactive.DC),
        zandp(samples.A2, Proactive.DN)
    )
))
...
Capacity.Table <- data.frame(rbind(
    A.PM.minus.Control=c(zandp(samples.A2, Capacity.ccAC),
                         zandp(samples.A2, Capacity.nnAN),
                         zandp(samples.A2, Capacity.nnAC),
                         zandp(samples.A2, Capacity.ccAN)
    ),

    B.PM.minus.Control=c(zandp(samples.A2, Capacity.ccBC),
                         zandp(samples.A2, Capacity.nnBN),
                         zandp(samples.A2, Capacity.nnBC),
                         zandp(samples.A2, Capacity.ccBN)
    ),

    C.PM.minus.Control=c(zandp(samples.A2, Capacity.ccCC),
                         zandp(samples.A2, Capacity.nnCN),
                         zandp(samples.A2, Capacity.nnCC),
                         zandp(samples.A2, Capacity.ccCN)
    ),

    D.PM.minus.Control=c(zandp(samples.A2, Capacity.ccDC),
                         zandp(samples.A2, Capacity.nnDN),
                         zandp(samples.A2, Capacity.nnDC),
                         zandp(samples.A2, Capacity.ccDN)
    )
))
...
ProactiveTP.Table <- data.frame(rbind(
    PM.Cost.A.minus.B=c(zandp(samples.A2, ProactiveTP.ABC),zandp(samples.A2, ProactiveTP.ABN)
    ),

    PM.Cost.B.minus.C=c(zandp(samples.A2, ProactiveTP.BCC),zandp(samples.A2, ProactiveTP.BCN)
    ),

    PM.Cost.C.minus.D=c(zandp(samples.A2, ProactiveTP.CDC),zandp(samples.A2, ProactiveTP.CDN)
    )
))
...
Effort.by.TP.Table.Wide <- data.frame(rbind(
    Conflict=c(zandp(samples.A2, Effort.TPccCABC),
               zandp(samples.A2, Effort.TPccCBCC),
               zandp(samples.A2, Effort.TPccCCDC),

               zandp(samples.A2, Effort.TPccPMABC),
               zandp(samples.A2, Effort.TPccPMBCC),
               zandp(samples.A2, Effort.TPccPMCDC)
    ),

    Nonconflict=c(zandp(samples.A2, Effort.TPnnCABN),
                  zandp(samples.A2, Effort.TPnnCBCN),
                  zandp(samples.A2, Effort.TPnnCCDN),

                  zandp(samples.A2, Effort.TPnnPMABN),
                  zandp(samples.A2, Effort.TPnnPMBCN),
                  zandp(samples.A2, Effort.TPnnPMCDN)
    ),

    Conflict.FA=c(zandp(samples.A2, Effort.TPnnCABC),
                  zandp(samples.A2, Effort.TPnnCBCC),
                  zandp(samples.A2, Effort.TPnnCCDC),

                  zandp(samples.A2, Effort.TPnnPMABC),
                  zandp(samples.A2, Effort.TPnnPMBCC),
                  zandp(samples.A2, Effort.TPnnPMCDC)
    ),

    Nonconflict.FA=c(zandp(samples.A2, Effort.TPccCABN),
                     zandp(samples.A2, Effort.TPccCBCN),
                     zandp(samples.A2, Effort.TPccCCDN),

                     zandp(samples.A2, Effort.TPccPMABN),
                     zandp(samples.A2, Effort.TPccPMBCN),
                     zandp(samples.A2, Effort.TPccPMCDN)
    ),
    PM.to.PM=c("-","-","-",
               zandp(samples.A2, Effort.TPppPMABP),
               zandp(samples.A2, Effort.TPppPMBCP),
               zandp(samples.A2, Effort.TPppPMCDP)
    ),
    Conflict.to.PMC=c("-","-","-",
                      zandp(samples.A2, Effort.TPpcPMABC),
                      zandp(samples.A2, Effort.TPpcPMBCC),
                      zandp(samples.A2, Effort.TPpcPMCDC)
    ),
    Nonconflict.to.PMN=c("-","-","-",
                         zandp(samples.A2, Effort.TPpnPMABN),
                         zandp(samples.A2, Effort.TPpnPMBCN),
                         zandp(samples.A2, Effort.TPpnPMCDN)
    ),
    Conflict.to.PMN=c("-","-","-",
                      zandp(samples.A2, Effort.TPpnPMABC),
                      zandp(samples.A2, Effort.TPpnPMBCC),
                      zandp(samples.A2, Effort.TPpnPMCDC)
    ),
    Nonconflict.to.PMC=c("-","-","-",
                         zandp(samples.A2, Effort.TPpcPMABN),
                         zandp(samples.A2, Effort.TPpcPMBCN),
                         zandp(samples.A2, Effort.TPpcPMCDN)
    )
))
...
#     Control.A.minus.B=c(zandp(samples.A2, Effort.TPccCABC),
#                         zandp(samples.A2, Effort.TPnnCABN),
#                         zandp(samples.A2, Effort.TPnnCABC),
#                         zandp(samples.A2, Effort.TPccCABN),
#                         "-","-","-","-","-"
#                         ),
#     Control.B.minus.C=c(zandp(samples.A2, Effort.TPccCBCC),
#                         zandp(samples.A2, Effort.TPnnCBCN),
#                         zandp(samples.A2, Effort.TPnnCBCC),
#                         zandp(samples.A2, Effort.TPccCBCN),
#                         "-","-","-","-","-"
#     ),
#     Control.C.minus.D=c(zandp(samples.A2, Effort.TPccCCDC),
#                         zandp(samples.A2, Effort.TPnnCCDN),
#                         zandp(samples.A2, Effort.TPnnCCDC),
#                         zandp(samples.A2, Effort.TPccCCDN),
#                         "-","-","-","-","-"
#     ),
#     PM.A.minus.B=c(zandp(samples.A2, Effort.TPccPMABC),
#                    zandp(samples.A2, Effort.TPnnPMABN),
#                    zandp(samples.A2, Effort.TPnnPMABC),
#                    zandp(samples.A2, Effort.TPccPMABN),
#
...
#
#     ),
#     PM.B.minus.C=c(zandp(samples.A2, Effort.TPccPMBCC),
#                    zandp(samples.A2, Effort.TPnnPMBCN),
#                    zandp(samples.A2, Effort.TPnnPMBCC),
#                    zandp(samples.A2, Effort.TPccPMBCN),
#
...
#                    zandp(samples.A2, Effort.TPpcPMBCN)
#     ),
#     PM.C.minus.D=c(zandp(samples.A2, Effort.TPccPMCDC),
#                    zandp(samples.A2, Effort.TPnnPMCDN),
#                    zandp(samples.A2, Effort.TPnnPMCDC),
#                    zandp(samples.A2, Effort.TPccPMCDN),
#
...
#     PM.to.PM=c(zandp(samples.A2, Effort.TPppPMABP),
#                zandp(samples.A2, Effort.TPppPMBCP),
#                zandp(samples.A2, Effort.TPppPMCDP),
#                "-","-","-"
#     ),
#
#     Conflict.to.PM=c(zandp(samples.A2, Effort.TPpcPMABC),
#                      zandp(samples.A2, Effort.TPpcPMBCC),
#                      zandp(samples.A2, Effort.TPpcPMCDC),
#
#                      zandp(samples.A2, Effort.TPpcPMABN),
...
#     ),
#
#     Nonconflict.to.PM=c(zandp(samples.A2, Effort.TPpnPMABN),
#                         zandp(samples.A2, Effort.TPpnPMBCN),
#                         zandp(samples.A2, Effort.TPpnPMCDN),
#
#                         zandp(samples.A2, Effort.TPpnPMABC),
