...
```{r Fit Plot: Accuracy Ongoing, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Fits.Accuracy.Ongoing}Model Fits to Ongoing Task Accuracy"}
...
```{r Fit Plot: Accuracy PM, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Fits.Accuracy.PM}Model Fits to PM Accuracy"}
...
```{r Fit Plot: RT Ongoing Low Load, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Fits.RT.Ongoing.2}Model Fits to Ongoing Task RT (Low Trial Load)"}
...
```{r Fit Plot: RT Ongoing High Load, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Fits.RT.Ongoing.5}Model Fits to Ongoing Task RT (High Trial Load)"}
...
```{r Fit Plot: RT PM, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Fits.RT.PM}Model Fits to PM RT"}
...
Figure \ref{fig:Fits.NR} shows observed versus predicted nonresponse proportions. As shown, the model's predicted nonresponse proportions closely match the empirical nonresponse proportions. This gives us confidence that the slight model misspecification due to fitting to truncated data is not of concern in terms of the models predictive validity. 

```{r Fit Plot: Nonresponses, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Fits.NR}Model Fits to Nonresponse Proportions"}
...
```{r Plot: Capacity PM, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Effort.Arousal.PM}Ongoing Task Accumulation Rates by PM Block"}
...
```{r Plot: Proactive Control PM, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Proactive.Control.PM}Ongoing Task Thresholds by PM Block"}
...
```{r Plot: Proactive Control TP, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Proactive.Control.TP}Ongoing Task and PM Thresholds by Time Pressure"}
...
```{r Plot: Reactive Inhibition, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Reactive.Inhibition}Ongoing Task Accumulation Rates for PM and Non-PM Stimuli"}
...
set.caption("General Linear Mixed Effects Model: PM Block and Time Pressure as Predictors of Self-Reported Task Effort")
...
set.caption("General Linear Mixed Effects Model: Accumulation Rates as Predictors of Self-Reported Task Effort")
pander(TLX.lm.TABLE.E1)
...
```{r Plot: Effort/Arousal, echo=FALSE, out.width='100%', fig.cap="\\label{fig:Effort.Arousal.TP}Ongoing Task Accumulation Rates by Time Pressure"}
...
Finally, we examine the overall importance of proactive and reactive inhibition mechanisms in terms of how well the model fits observed accuracy and RT. As shown in Figure \ref{fig:ModelExpTPContrastsPM_ControlMech}, the full model (which includes both proactive and reactive control mechanisms) provided an almost exact fit to accuracy (`r options(digits = 2); Cog.Control.Percent.Table$Cog.Control.Percent[1]`% of the effect), while slightly overpredicting overall RT (RT overpredicted by `r options(digits = 2); Cog.Control.Percent.Table$Diff.from.100[2]`%).

In contrast, when proactive control is removed and the model only allows for reactive control, we get severe mis-fit to both overall accuracy and RT, the model underpredicting both by a large margin (accuracy underpredicted by `r options(digits = 2); abs(Cog.Control.Percent.Table$Diff.from.100[3])`%, RT underpredicted by `r options(digits = 2); abs(Cog.Control.Percent.Table$Diff.from.100[4])`%). Similarly, when reactive control is removed from the model and only proactive control included, we get less severe but still substantial underprediction of both accuracy and RT (accuracy underpredicted by `r options(digits = 2); abs(Cog.Control.Percent.Table$Diff.from.100[5])`%, RT underpredicted by `r options(digits = 2); abs(Cog.Control.Percent.Table$Diff.from.100[6])`%).

Lastly, when both proactive and reactive control mechanisms are turned off (i.e., the model includes neither mechanism), the model produces the worst overall fit to accuracy and RT, underpredicting both by a larger margin than either the proactive-only or reactive-only models (accuracy underpredicted by `r options(digits = 2); abs(Cog.Control.Percent.Table$Diff.from.100[7])`%, RT underpredicted by `r options(digits = 2); abs(Cog.Control.Percent.Table$Diff.from.100[8])`%). This provides convincing evidence that both proactive and reactive control mechanisms play critical and complimentary roles in accounting for ongoing task costs associated with different PM and time pressure demands. 
...
set.caption("Generalised Linear Mixed Effects Model: Ongoing Task Accuracy")
...
set.caption("General Linear Mixed Effects Model: Ongoing Task RT")
...
set.caption("Generalised Linear Mixed Effects Model: PM Accuracy")
...
set.caption("General Linear Mixed Effects Model: PM RT")
...
set.caption("Generalised Linear Mixed Effects Model: Manifest Reactive Control Check")
...
```{r Plot: Model Exploration PM Contrasts, echo=FALSE, out.width='100%', fig.cap="\\label{fig:PM.Control.Contrast.Plots}Model Exploration: Comparison of Accuracy and RT Fits for Models with Thresholds or Rates Averaged over PM-Control Blocks"}

knitr::include_graphics("figures/E1/E1.PM.Control.Contrast.Plots.png")

```

```{r Plot: Model Exploration TP Contrasts, echo=FALSE, out.width='100%', fig.cap="\\label{fig:ModelExpTPContrastsOngoing}Model Exploration: Comparison of Ongoing Task Accuracy and RT Fits for Models with Thresholds or Rates Averaged over Time Pressure Conditions"}

knitr::include_graphics("figures/E1/E1.ModelExpTPContrastsOngoing.png")

```

```{r Plot: Model Exploration TP Contrasts PM & Control Mechanisms, echo=FALSE, out.width='100%', fig.cap="\\label{fig:ModelExpTPContrastsPM_ControlMech}Top Panel: Model Exploration: Comparison of PM Accuracy and RT Fits for Models with Thresholds or Rates Averaged over Time Pressure Conditions. Bottom Panel: Model Exploration: Comparison of Accuracy and RT Fits for Models with Cognitive Control Mechanisms Selectively Removed."}
