...
# # # Load samples object # # #
#
load("C:/Users/Russell Boag/Documents/GitHub/DMCATC/samples/A1.block.B.V_cond.B.V.PMV.samples.RData")  # Samples object
...
Reactive.Table <- data.frame(rbind(
    A.NonPM.minus.PM=c(zandp(samples.A1, Reactive.ccAC),
        zandp(samples.A1, Reactive.nnAN),
        zandp(samples.A1, Reactive.nnAC),
        zandp(samples.A1, Reactive.ccAN)
        ),

    B.NonPM.minus.PM=c(zandp(samples.A1, Reactive.ccBC),
        zandp(samples.A1, Reactive.nnBN),
        zandp(samples.A1, Reactive.nnBC),
        zandp(samples.A1, Reactive.ccBN)
        ),

    C.NonPM.minus.PM=c(zandp(samples.A1, Reactive.ccCC),
        zandp(samples.A1, Reactive.nnCN),
        zandp(samples.A1, Reactive.nnCC),
        zandp(samples.A1, Reactive.ccCN)
        ),

    D.NonPM.minus.PM=c(zandp(samples.A1, Reactive.ccDC),
        zandp(samples.A1, Reactive.nnDN),
        zandp(samples.A1, Reactive.nnDC),
        zandp(samples.A1, Reactive.ccDN)
        )
))
...
Proactive.Table <- data.frame(rbind(
    A.PM.minus.Control=c(zandp(samples.A1, Proactive.AC),
        zandp(samples.A1, Proactive.AN)
    ),

    B.PM.minus.Control=c(zandp(samples.A1, Proactive.BC),
        zandp(samples.A1, Proactive.BN)
    ),

    C.PM.minus.Control=c(zandp(samples.A1, Proactive.CC),
        zandp(samples.A1, Proactive.CN)
    ),

    D.PM.minus.Control=c(zandp(samples.A1, Proactive.DC),
        zandp(samples.A1, Proactive.DN)
    )
))
...
Capacity.Table <- data.frame(rbind(
    A.PM.minus.Control=c(zandp(samples.A1, Capacity.ccAC),
                         zandp(samples.A1, Capacity.nnAN),
                         zandp(samples.A1, Capacity.nnAC),
                         zandp(samples.A1, Capacity.ccAN)
    ),

    B.PM.minus.Control=c(zandp(samples.A1, Capacity.ccBC),
                         zandp(samples.A1, Capacity.nnBN),
                         zandp(samples.A1, Capacity.nnBC),
                         zandp(samples.A1, Capacity.ccBN)
    ),

    C.PM.minus.Control=c(zandp(samples.A1, Capacity.ccCC),
                         zandp(samples.A1, Capacity.nnCN),
                         zandp(samples.A1, Capacity.nnCC),
                         zandp(samples.A1, Capacity.ccCN)
    ),

    D.PM.minus.Control=c(zandp(samples.A1, Capacity.ccDC),
                         zandp(samples.A1, Capacity.nnDN),
                         zandp(samples.A1, Capacity.nnDC),
                         zandp(samples.A1, Capacity.ccDN)
    )
))
...
ProactiveTP.Table <- data.frame(rbind(
    PM.Cost.A.minus.B=c(zandp(samples.A1, ProactiveTP.ABC),zandp(samples.A1, ProactiveTP.ABN)
    ),

    PM.Cost.B.minus.C=c(zandp(samples.A1, ProactiveTP.BCC),zandp(samples.A1, ProactiveTP.BCN)
    ),

    PM.Cost.C.minus.D=c(zandp(samples.A1, ProactiveTP.CDC),zandp(samples.A1, ProactiveTP.CDN)
    )
))
...
Effort.by.TP.Table.Wide <- data.frame(rbind(
    Conflict=c(zandp(samples.A1, Effort.TPccCABC),
               zandp(samples.A1, Effort.TPccCBCC),
               zandp(samples.A1, Effort.TPccCCDC),

               zandp(samples.A1, Effort.TPccPMABC),
               zandp(samples.A1, Effort.TPccPMBCC),
               zandp(samples.A1, Effort.TPccPMCDC)
    ),

    Nonconflict=c(zandp(samples.A1, Effort.TPnnCABN),
                  zandp(samples.A1, Effort.TPnnCBCN),
                  zandp(samples.A1, Effort.TPnnCCDN),

                  zandp(samples.A1, Effort.TPnnPMABN),
                  zandp(samples.A1, Effort.TPnnPMBCN),
                  zandp(samples.A1, Effort.TPnnPMCDN)
    ),

    Conflict.FA=c(zandp(samples.A1, Effort.TPnnCABC),
                  zandp(samples.A1, Effort.TPnnCBCC),
                  zandp(samples.A1, Effort.TPnnCCDC),

                  zandp(samples.A1, Effort.TPnnPMABC),
                  zandp(samples.A1, Effort.TPnnPMBCC),
                  zandp(samples.A1, Effort.TPnnPMCDC)
    ),

    Nonconflict.FA=c(zandp(samples.A1, Effort.TPccCABN),
                     zandp(samples.A1, Effort.TPccCBCN),
                     zandp(samples.A1, Effort.TPccCCDN),

                     zandp(samples.A1, Effort.TPccPMABN),
                     zandp(samples.A1, Effort.TPccPMBCN),
                     zandp(samples.A1, Effort.TPccPMCDN)
    ),
    PM.to.PM=c("-","-","-",
               zandp(samples.A1, Effort.TPppPMABP),
               zandp(samples.A1, Effort.TPppPMBCP),
               zandp(samples.A1, Effort.TPppPMCDP)
    ),
    Conflict.to.PMC=c("-","-","-",
                      zandp(samples.A1, Effort.TPpcPMABC),
                      zandp(samples.A1, Effort.TPpcPMBCC),
                      zandp(samples.A1, Effort.TPpcPMCDC)
    ),
    Nonconflict.to.PMN=c("-","-","-",
                         zandp(samples.A1, Effort.TPpnPMABN),
                         zandp(samples.A1, Effort.TPpnPMBCN),
                         zandp(samples.A1, Effort.TPpnPMCDN)
    ),
    Conflict.to.PMN=c("-","-","-",
                      zandp(samples.A1, Effort.TPpnPMABC),
                      zandp(samples.A1, Effort.TPpnPMBCC),
                      zandp(samples.A1, Effort.TPpnPMCDC)
    ),
    Nonconflict.to.PMC=c("-","-","-",
                         zandp(samples.A1, Effort.TPpcPMABN),
                         zandp(samples.A1, Effort.TPpcPMBCN),
                         zandp(samples.A1, Effort.TPpcPMCDN)
    )
))
...
#     Control.A.minus.B=c(zandp(samples.A1, Effort.TPccCABC),
#                         zandp(samples.A1, Effort.TPnnCABN),
#                         zandp(samples.A1, Effort.TPnnCABC),
#                         zandp(samples.A1, Effort.TPccCABN),
#                         "-","-","-","-","-"
#                         ),
#     Control.B.minus.C=c(zandp(samples.A1, Effort.TPccCBCC),
#                         zandp(samples.A1, Effort.TPnnCBCN),
#                         zandp(samples.A1, Effort.TPnnCBCC),
#                         zandp(samples.A1, Effort.TPccCBCN),
#                         "-","-","-","-","-"
#     ),
#     Control.C.minus.D=c(zandp(samples.A1, Effort.TPccCCDC),
#                         zandp(samples.A1, Effort.TPnnCCDN),
#                         zandp(samples.A1, Effort.TPnnCCDC),
#                         zandp(samples.A1, Effort.TPccCCDN),
#                         "-","-","-","-","-"
#     ),
#     PM.A.minus.B=c(zandp(samples.A1, Effort.TPccPMABC),
#                    zandp(samples.A1, Effort.TPnnPMABN),
#                    zandp(samples.A1, Effort.TPnnPMABC),
#                    zandp(samples.A1, Effort.TPccPMABN),
#
...
#
#     ),
#     PM.B.minus.C=c(zandp(samples.A1, Effort.TPccPMBCC),
#                    zandp(samples.A1, Effort.TPnnPMBCN),
#                    zandp(samples.A1, Effort.TPnnPMBCC),
#                    zandp(samples.A1, Effort.TPccPMBCN),
#
...
#                    zandp(samples.A1, Effort.TPpcPMBCN)
#     ),
#     PM.C.minus.D=c(zandp(samples.A1, Effort.TPccPMCDC),
#                    zandp(samples.A1, Effort.TPnnPMCDN),
#                    zandp(samples.A1, Effort.TPnnPMCDC),
#                    zandp(samples.A1, Effort.TPccPMCDN),
#
...
#     PM.to.PM=c(zandp(samples.A1, Effort.TPppPMABP),
#                zandp(samples.A1, Effort.TPppPMBCP),
#                zandp(samples.A1, Effort.TPppPMCDP),
#                "-","-","-"
#     ),
#
#     Conflict.to.PM=c(zandp(samples.A1, Effort.TPpcPMABC),
#                      zandp(samples.A1, Effort.TPpcPMBCC),
#                      zandp(samples.A1, Effort.TPpcPMCDC),
#
#                      zandp(samples.A1, Effort.TPpcPMABN),
...
#     ),
#
#     Nonconflict.to.PM=c(zandp(samples.A1, Effort.TPpnPMABN),
#                         zandp(samples.A1, Effort.TPpnPMBCN),
#                         zandp(samples.A1, Effort.TPpnPMCDN),
#
#                         zandp(samples.A1, Effort.TPpnPMABC),
